...
default_hooks = dict(
    checkpoint=dict(
        interval=2, rule='greater', save_best='mIoU', type='CheckpointHook'),
...
model = dict(
    bg_idx=4,
    clip_type='CLIP',
    cls_token_lambda=-3e-05,
    feature_cls_token_lambda=-6e-06,
    feature_up=True,
    feature_up_cfg=dict(
        model_name='jbu_one',
    ...
    vit_type='ViT-B/16')
optim_wrapper = dict(
    optimizer=dict(lr=1e-05, type='Adam'), type='OptimWrapper')
