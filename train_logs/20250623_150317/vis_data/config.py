...
model = dict(
    ...
    vit_type='ViT-B/16')
optim_wrapper = dict(
    optimizer=dict(lr=0.005, type='Adam'), type='OptimWrapper')
...
test_dataloader = dict(
    ...
    dataset=dict(
        data_prefix=dict(
            img_path='data/VDD/test/src', seg_map_path='data/VDD/test/gt'),
        data_root='',
        pipeline=[
...
train_cfg = dict(max_epochs=200, type='EpochBasedTrainLoop', val_interval=20)
train_dataloader = dict(
    batch_size=1,
    dataset=dict(
        data_prefix=dict(
            img_path='data/VDD/train/src', seg_map_path='data/VDD/train/gt'),
        data_root='',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations'),
...
val_dataloader = dict(
    ...
    dataset=dict(
        data_prefix=dict(
            img_path='data/VDD/val/src', seg_map_path='data/VDD/val/gt'),
        data_root='',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(keep_ratio=True, scale=(
