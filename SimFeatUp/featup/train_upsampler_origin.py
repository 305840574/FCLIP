...
class SimFeatUp(pl.LightningModule):
    def __init__(self,
        ...
        self.model, self.patch_size, self.dim = get_featurizer(model_type, activation_type,isfusion=False, num_classes=1000)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step == 6000:
            for param_group in opt.param_groups:
                param_group['lr'] *= 0.3
            print(f"[Info] Learning rate reduced to {param_group['lr']} at global_step {self.global_step}")
        '''
